...
# Filter for Ian Plamondom tutors using MasterTutor tab
@st.cache_data(ttl=60)
...
#     st.write(master_tutor_df.columns.tolist())
    ...
    if page == "KPI Table":
        ...
        leader_name = "Ian Plamondon"  # can later make this a dropdown if desired
        ...
        if len(date_ranges_sorted) < 2:
            ...
            # Filter for Ian' team in both time periods
    ...
    #         )
        ...
        # Load the full dashboard metrics table
        ...
        if dashboard_df.empty:
            ...
            # Filter to selected Faculty Leader
            team_dashboard_df = dashboard_df[
                dashboard_df["Faculty Leader Name"] == leader_name
            ].copy()
            ...
            kpi_thresholds = {
                ...
                "Prep Time %": (0.20, 0.10),  # lower is better
            ...
            # -------------------------
            # DOWNLOAD BUTTON (Excel with formatting)
            # -------------------------
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                ...
                fmt_percentage   = workbook.add_format({'num_format': '0.00%', 'align': 'center'})  # Excel % with 2 decimals
                fmt_decimal      = workbook.add_format({'num_format': '0.00', 'align': 'center'})   # 2 decimals
                ...
                for col_num, col_name in enumerate(team_dashboard_df.columns):
                    if col_name in ["% to Delivery Target",
                                    "% to Availability Target",
                                    "Prep Time %",
                                    "% Parents Updates Done on Time",
                                    "% Sessions on Time",
                ...
                kpi_thresholds = {
                    "% to Delivery Target": (0.70, 0.80),
                    "% to Availability Target": (0.80, 1.00),
                    "Prep Time %": (0.20, 0.10),  # reversed logic
                    "% Parents Updates Done on Time": (0.75, 0.90),
                    "% Sessions on Time": (0.80, 0.95),
                    "% of Active Students with Progress Updates Completed": (0.50, 0.80)
                }
                ...
                for col_num, col_name in enumerate(team_dashboard_df.columns):
                    if col_name in kpi_thresholds:
                        ...
                        col_letter = chr(65 + col_num)  # assumes <26 columns
                        # Normal KPIs
            ...
            if not team_progress_df.empty:
                ...
                with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                    ...
                    for col_num, col_name in enumerate(team_progress_df.columns):
                        if "%" in col_name:  # simple heuristic for % columns
                            worksheet.set_column(col_num, col_num, 15, fmt_percentage)

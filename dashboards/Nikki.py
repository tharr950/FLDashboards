...
PROGRESS_SNAPSHOT_FILE = "nikki_progress_score_snapshots.csv"
...
WATCHLIST_FILE            = "nikki_watchlist.csv"
WATCHLIST_BASELINE_FILE   = "nikki_watchlist_baselines.csv"
WATCHLIST_NOTES_FILE      = "nikki_watchlist_notes.csv"
WATCHLIST_THRESHOLDS_FILE = "nikki_watchlist_thresholds.csv"
...
LOGIN_SNAPSHOT_FILE = "nikki_login_snapshot.csv"
LAST_SEEN_FILE      = "nikki_last_seen_data.csv"
...
def render_app(config):
    ...
    faculty_leader_name = "Nikki Pencak"
    master_tutor_df = load_master_tutor()
    annelies_tutors = master_tutor_df[(master_tutor_df["Faculty Leader"] == faculty_leader_name) & (~master_tutor_df["Full Name"].isin(["Nikki Pencak', 'Annelies de Groot', 'Ian Plamondon', 'Geoff St. Marie', 'Kristin Haase-Alvey"]))]["Full Name"].sort_values().dropna().unique().tolist()
    ...
    annelies_tutors = master_tutor_df[(master_tutor_df["Faculty Leader"] == "Nikki Pencak") & (~master_tutor_df["Full Name"].isin(["Nikki Pencak', 'Annelies de Groot', 'Ian Plamondon', 'Geoff St. Marie', 'Kristin Haase-Alvey"]))]["Full Name"].dropna().sort_values().tolist()
    ...
    if page == "🏠 Home":
        ...
        with st.spinner("Loading your briefing..."):
            ...
            try:
                ...
                home_arch_df = raw_arch_df[raw_arch_df["team_name"] == "Team Pencak"].copy()
            except Exception as e:
                home_arch_df = pd.DataFrame(); load_errors.append(f"Archivable data: {e}")

            try:
                raw_home_grades, _ = load_grades_data()
                home_grades_df = raw_home_grades[raw_home_grades["team_name"] == "Team Pencak"].copy()
            ...
            try:
                ...
                home_exam_df = raw_home_exam[raw_home_exam["team_name"] == "Team Pencak"].copy()
            ...
            try:
                ...
                team_avail_df = avail_df[avail_df["team"] == "Team Pencak"].copy()
            ...
            try:
                ...
                home_video_df = raw_video[
                    (raw_video["faculty leader"] == "Team Pencak") &
                    (raw_video["tutor"] != "Nikki Pencak")
                ].copy()
        ...
        if not home_arch_df.empty:
            ...
            if team_avail_df.empty:
                ...
                <div style='background:#f0fff4; border:1.5px solid #b2f5c8; border-radius:10px;
                    ...
                    ✅ <b>Availability Looks Good</b> — No tutors on Team Pencak have 7+ days
        ...
        try:
            _toggle_mismatch_df = load_buc_instruction_toggle_mismatch("Nikki Pencak")
        ...
        try:
            ...
            if not _featured_df.empty:
                _my_featured = _featured_df[_featured_df["faculty_leader"] == "Nikki Pencak"].copy()
        ...
        if not kpi_home_df.empty:
            ...
            latest_team = kpi_home_df[
                ...
                (kpi_home_df["Faculty Leader"] == "Nikki Pencak")]
            prev_team   = kpi_home_df[
                (kpi_home_df["Date Range Parsed"] == prev_kpi) &
                (kpi_home_df["Faculty Leader"] == "Nikki Pencak")] if pd.notna(prev_kpi) else pd.DataFrame()
            for m in kpi_metrics:
        ...
        _intro = (f"Here's your briefing for **{_today_str}**. "
                  f"Team Pencak has **{_total_active} active students**, "
...
# ─────────────────────────────────────────────
    ...
    if page == "👀 Watched Tutors":
        ...
        all_tutors_wl = sorted(
            master_tutor_df[master_tutor_df["Faculty Leader"] == "Nikki Pencak"]["Full Name"]
        ...
        with st.spinner("Loading watch list data..."):
            try:
                ...
                raw_wl_arch["should_archive"] = raw_wl_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                wl_arch_df = raw_wl_arch[raw_wl_arch["team_name"] == "Team Pencak"].copy()
            except Exception as e:
                wl_arch_df = pd.DataFrame(); load_errors_wl.append(f"Archivable: {e}")

            try:
                raw_wl_grades, _ = load_grades_data()
                wl_grades_df = raw_wl_grades[raw_wl_grades["team_name"] == "Team Pencak"].copy()
            ...
            try:
                ...
                wl_exam_df = raw_wl_exam[raw_wl_exam["team_name"] == "Team Pencak"].copy()
                for dc in ["first_session_day","most_recent_session","exam_date"]:
            ...
            try:
                ...
                wl_video_df = raw_wl_video[
                    (raw_wl_video["faculty leader"] == "Team Pencak") &
                    (raw_wl_video["tutor"] != "Nikki Pencak")
                ].copy()
    ...
    if page == "👤 Tutor Profile":
        ...
        with st.spinner(f"Loading data for {profile_tutor}…"):
            try:
                ...
                raw_p_arch["should_archive"] = raw_p_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                p_arch = raw_p_arch[
                    (raw_p_arch["team_name"] == "Team Pencak") &
            ...
            try:
                ...
                p_grades = raw_p_grades[
                    (raw_p_grades["team_name"] == "Team Pencak") &
            ...
            try:
                ...
                p_exam = raw_p_exam[
                    (raw_p_exam["team_name"] == "Team Pencak") &
            ...
            try:
                ...
                p_video_df = raw_p_video[
                    (raw_p_video["faculty leader"] == "Team Pencak") &
                ...
                if not p_video_df.empty:
                    all_video = raw_p_video[
                        (raw_p_video["faculty leader"] == "Team Pencak") &
                        (raw_p_video["tutor"] != "Nikki Pencak")
                    ].copy()
        ...
        if p_monthly_t.empty:
            ...
            team_names_p   = p_master[p_master["Faculty Leader"] == "Nikki Pencak"]["Full Name"].dropna()
    ...
    if page == "📹 Parent Update Videos":
        ...
        st.download_button(
            ...
            data=out_v,
            file_name=f"Parent_Update_Videos_{sel_tutor_v.replace(' ','_') if sel_tutor_v != 'All Tutors' else 'Nikki_Pencak'}.xlsx",
...
**Multiple tutors** is based on the last 30 days. When filtering to "Solo Tutor Only", only students where this tutor is their **only** tutor are shown.
        ...
        team_grades_df = raw_grades_df[raw_grades_df["team_name"] == "Team Pencak"].copy()

        if team_grades_df.empty:
            st.warning("No grades records found for Team Pencak.")
            st.stop()
        ...
        with tab_detail:
            ...
            if view_grades_df.empty:
                ...
                st.download_button(
                    ...
                    data=output_g,
                    file_name=f"Grades_Detail_{sel_tutor_g.replace(' ','_') if sel_tutor_g != 'All Tutors' else 'Nikki_Pencak'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if page == "Test Prep & Exams":
        ...
        try:
            _report_path = "data/exam_reports/Nikki_Pencak_exam_report.pdf"
...
- If test prep is covered in an **Academics session**: it is counted in delivered sessions, but Academics are not included for sessions scheduled in the future
        ...
        team_exam_df = raw_exam_df[raw_exam_df["team_name"] == "Team Pencak"].copy()
        if team_exam_df.empty:
            st.warning("No exam records found for Team Pencak.")
            st.stop()
        ...
        with tab_det_e:
            if view_exam_df.empty:
                ...
                st.download_button(
                    ...
                    data=out_e,
                    file_name=f"Exam_Detail_{sel_tutor_e.replace(' ','_') if sel_tutor_e != 'All Tutors' else 'Nikki_Pencak'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
**Tutors with no updates sent that week will appear with blank scores.**
        ...
        if team_scores_df.empty:
            st.warning(f"No scored progress updates found for Team Pencak.")
            st.stop()
        ...
        st.download_button(
            ...
            data=out_qs,
            file_name=f"Update_Quality_Scores_{sel_tutor.replace(' ','_') if sel_tutor != 'All Tutors' else 'Nikki_Pencak'}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    ...
    if page == "Archivable Students & Unscheduled Hours":
        ...
        raw_df["should_archive"] = raw_df["should_archive"].apply(
            lambda x: bool(x) if pd.notna(x) else False)
        full_team_df = raw_df[raw_df["team_name"] == "Team Pencak"].copy()
        if full_team_df.empty:
            st.warning("No records found for Team Pencak."); st.stop()
        ...
        with tab1:
            ...
            if archive_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Archivable_Students_Nikki_Pencak.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        with tab2:
            ...
            if unsched_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Unscheduled_Hours_Nikki_Pencak.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        if not nps_start_val:
            ...
            try:
                with st.spinner("Loading NPS data..."):
                    nps_df = load_nps_scores(nps_start_val, nps_end_val, "Team Pencak")
        ...
        if not pu_as_of_val:
            ...
            try:
                with st.spinner("Loading progress update data..."):
                    pu_df = load_progress_updates(pu_as_of_val, pu_since_val, "Team Pencak")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                pu_df = pd.DataFrame()
        ...
        if not ppw_start:
            ...
            try:
                with st.spinner("Loading PPW data..."):
                    ppw_df = load_ppw_data(ppw_start, ppw_end, "Team Pencak")
                st.caption(f"Debug: {len(ppw_df)} rows, dates {ppw_start} to {ppw_end}, team: Nikki Pencak")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                ppw_df = pd.DataFrame()
            if ppw_df.empty:
    ...
    if page == "Annual Reviews":
        ...
        if selected_annual_tutor:
            ...
            if not tutor_review.empty:
                ...
                team_df  = annual_review_df[annual_review_df["fl"] == "Nikki Pencak"]
                ...
                team_monthly_metric_df = monthly_metric_annual_review_df[
                    monthly_metric_annual_review_df["Faculty Leader"] == "Nikki Pencak"]
    ...
    if page == "KPI Trends":
        ...
        if selected_tutor:
            ...
            annelies_team = master_df[master_df["Faculty Leader"] == "Nikki Pencak"]["Full Name"].dropna()
    ...
    # ─────────────────────────────────────────────────────────────────────────
        ...
        team_tutors_df = master_tutor_df[
            (master_tutor_df["Faculty Leader"] == "Nikki Pencak") &
            (~master_tutor_df["Full Name"].isin(["Nikki Pencak"]))
        ].copy()

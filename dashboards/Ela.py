...
PROGRESS_SNAPSHOT_FILE = "ela_progress_score_snapshots.csv"
...
LAST_SEEN_FILE      = "ela_last_seen_data.csv"
...
def render_app(config):
    ...
    annelies_tutors = master_tutor_df[(master_tutor_df["Faculty Leader"] == "Ela Cross") & (~master_tutor_df["Full Name"].isin(["Ela Cross', 'Annelies de Groot', 'Ian Plamondon', 'Geoff St. Marie', 'Kristin Haase-Alvey"]))]["Full Name"].dropna().sort_values().tolist()
    ...
    if page == "🏠 Home":
        ...
        with st.spinner("Loading your briefing..."):
            ...
            try:
                ...
                home_arch_df = raw_arch_df[raw_arch_df["team_name"] == "Team Cross"].copy()
            except Exception as e:
                home_arch_df = pd.DataFrame(); load_errors.append(f"Archivable data: {e}")

            try:
                raw_home_grades, _ = load_grades_data()
                home_grades_df = raw_home_grades[raw_home_grades["team_name"] == "Team Cross"].copy()
            ...
            try:
                ...
                home_exam_df = raw_home_exam[raw_home_exam["team_name"] == "Team Cross"].copy()
            ...
            try:
                ...
                team_avail_df = avail_df[avail_df["team"] == "Team Cross"].copy()
        ...
        if not home_arch_df.empty:
            ...
            if team_avail_df.empty:
                ...
                <div style='background:#f0fff4; border:1.5px solid #b2f5c8; border-radius:10px;
                    ...
                    ✅ <b>Availability Looks Good</b> — No tutors on Team Cross have 7+ days
        ...
        try:
            _low_del_df = load_low_delivery_not_accepting("Ela Cross")
        ...
        try:
            _low_avail_df = load_low_delivery_low_availability("Ela Cross")
        ...
        try:
            _toggle_mismatch_df = load_buc_instruction_toggle_mismatch("Ela Cross")
        ...
        if not kpi_home_df.empty:
            ...
            latest_team = kpi_home_df[
                ...
                (kpi_home_df["Faculty Leader"] == "Ela Cross")]
            prev_team   = kpi_home_df[
                (kpi_home_df["Date Range Parsed"] == prev_kpi) &
                (kpi_home_df["Faculty Leader"] == "Ela Cross")] if pd.notna(prev_kpi) else pd.DataFrame()
            for m in kpi_metrics:
        ...
        _intro = (f"Here's your briefing for **{_today_str}**. "
                  f"Team Cross has **{_total_active} active students**, "
...
# ─────────────────────────────────────────────
    ...
    if page == "👀 Watched Tutors":
        ...
        all_tutors_wl = sorted(
            master_tutor_df[master_tutor_df["Faculty Leader"] == "Ela Cross"]["Full Name"]
        ...
        with st.spinner("Loading watch list data..."):
            try:
                ...
                raw_wl_arch["should_archive"] = raw_wl_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                wl_arch_df = raw_wl_arch[raw_wl_arch["team_name"] == "Team Cross"].copy()
            except Exception as e:
                wl_arch_df = pd.DataFrame(); load_errors_wl.append(f"Archivable: {e}")

            try:
                raw_wl_grades, _ = load_grades_data()
                wl_grades_df = raw_wl_grades[raw_wl_grades["team_name"] == "Team Cross"].copy()
            ...
            try:
                ...
                wl_exam_df = raw_wl_exam[raw_wl_exam["team_name"] == "Team Cross"].copy()
                for dc in ["first_session_day","most_recent_session","exam_date"]:
    ...
    if page == "👤 Tutor Profile":
        ...
        try:
            _ld_flag_df = load_low_delivery_not_accepting("Ela Cross")
        ...
        with st.spinner(f"Loading data for {profile_tutor}…"):
            try:
                ...
                raw_p_arch["should_archive"] = raw_p_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                p_arch = raw_p_arch[
                    (raw_p_arch["team_name"] == "Team Cross") &
            ...
            try:
                ...
                p_grades = raw_p_grades[
                    (raw_p_grades["team_name"] == "Team Cross") &
            ...
            try:
                ...
                p_exam = raw_p_exam[
                    (raw_p_exam["team_name"] == "Team Cross") &
        ...
        if p_monthly_t.empty:
            ...
            team_names_p   = p_master[p_master["Faculty Leader"] == "Ela Cross"]["Full Name"].dropna()
    ...
    if page == "📹 Parent Update Videos":
        ...
        st.download_button(
            ...
            data=out_v,
            file_name=f"Parent_Update_Videos_{sel_tutor_v.replace(' ','_') if sel_tutor_v != 'All Tutors' else 'Ela_Cross'}.xlsx",
...
- If test prep is covered in an **Academics session**: it is counted in delivered sessions, but Academics are not included for sessions scheduled in the future
        ...
        team_exam_df = raw_exam_df[raw_exam_df["team_name"] == "Team Cross"].copy()
        if team_exam_df.empty:
            st.warning("No exam records found for Team Cross.")
            st.stop()
...
**Tutors with no updates sent that week will appear with blank scores.**
        ...
        if team_scores_df.empty:
            st.warning(f"No scored progress updates found for Team Cross.")
            st.stop()
        ...
        st.download_button(
            ...
            data=out_qs,
            file_name=f"Update_Quality_Scores_{sel_tutor.replace(' ','_') if sel_tutor != 'All Tutors' else 'Ela_Cross'}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    ...
    if page == "Archivable Students & Unscheduled Hours":
        ...
        if full_team_df.empty:
            st.warning("No records found for Team Cross."); st.stop()
        ...
        if not nps_start_val:
            ...
            try:
                with st.spinner("Loading NPS data..."):
                    nps_df = load_nps_scores(nps_start_val, nps_end_val, "Team Cross")
        ...
        if not pu_as_of_val:
            ...
            try:
                with st.spinner("Loading progress update data..."):
                    pu_df = load_progress_updates(pu_as_of_val, pu_since_val, "Team Cross")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                pu_df = pd.DataFrame()
    ...
    if page == "🚫 Restricted Status":
        ...
        try:
            ...
            restr_df = restr_df[restr_df["faculty_leader"] == "Ela Cross"].copy()
        except Exception as e:
    ...
    if page == "📅 Meetings":
        ...
        try:
            ...
            mtg_df = mtg_df[mtg_df["faculty_leader"] == "Ela Cross"].copy()
        except Exception as e:
        ...
        if not ppw_start:
            ...
            try:
                with st.spinner("Loading PPW data..."):
                    ppw_df = load_ppw_data(ppw_start, ppw_end, "Team Cross")
                st.caption(f"Debug: {len(ppw_df)} rows, dates {ppw_start} to {ppw_end}, team: Ela Cross")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                ppw_df = pd.DataFrame()
            if ppw_df.empty:
    ...
    if page == "Annual Reviews":
        ...
        if selected_annual_tutor:
            ...
            if not tutor_review.empty:
                ...
                team_df  = annual_review_df[annual_review_df["fl"] == "Ela Cross"]
                ...
                team_monthly_metric_df = monthly_metric_annual_review_df[
                    monthly_metric_annual_review_df["Faculty Leader"] == "Ela Cross"]
    ...
    if page == "KPI Trends":
        ...
        if selected_tutor:
            ...
            annelies_team = master_df[master_df["Faculty Leader"] == "Ela Cross"]["Full Name"].dropna()

...
SNAPSHOT_HISTORY_FILE = "annelies_archive_snapshots_history.csv"
GRADES_SNAPSHOT_FILE = "annelies_grades_snapshots.csv"
EXAMS_SNAPSHOT_FILE  = "annelies_exams_snapshots.csv"
VIDEO_SNAPSHOT_FILE  = "annelies_video_snapshots.csv"
PROGRESS_SNAPSHOT_FILE = "annelies_progress_score_snapshots.csv"
...
LAST_SEEN_FILE      = "annelies_last_seen_data.csv"
...
def render_app(config):
    ...
    annelies_tutors = master_tutor_df[(master_tutor_df["Faculty Leader"] == "Annelies de Groot") & (~master_tutor_df["Full Name"].isin(["Ela Cross', 'Annelies de Groot', 'Ian Plamondon', 'Geoff St. Marie', 'Kristin Haase-Alvey"]))]["Full Name"].dropna().sort_values().tolist()
    ...
    if page == "🏠 Home":
        ...
        with st.spinner("Loading your briefing..."):
            ...
            try:
                ...
                home_video_df = raw_video[
                    (raw_video["faculty leader"] == "Team De Groot") &
                    (raw_video["tutor"] != "Annelies de Groot")
                ].copy()
        ...
        try:
            _low_del_df = load_low_delivery_not_accepting("Annelies de Groot")
        ...
        try:
            _low_avail_df = load_low_delivery_low_availability("Annelies de Groot")
        ...
        try:
            _toggle_mismatch_df = load_buc_instruction_toggle_mismatch("Annelies de Groot")
        ...
        try:
            ...
            if not _featured_df.empty:
                _my_featured = _featured_df[_featured_df["faculty_leader"] == "Annelies de Groot"].copy()
...
# ─────────────────────────────────────────────
        ...
        with st.spinner("Loading watch list data..."):
            ...
            try:
                ...
                wl_video_df = raw_wl_video[
                    (raw_wl_video["faculty leader"] == "Team De Groot") &
                    (raw_wl_video["tutor"] != "Annelies de Groot")
                ].copy()
    ...
    if page == "👤 Tutor Profile":
        ...
        try:
            _ld_flag_df = load_low_delivery_not_accepting("Annelies de Groot")
        ...
        with st.spinner(f"Loading data for {profile_tutor}…"):
            ...
            try:
                ...
                p_video_df = raw_p_video[
                    (raw_p_video["faculty leader"] == "Team De Groot") &
                ...
                if not p_video_df.empty:
                    all_video = raw_p_video[
                        (raw_p_video["faculty leader"] == "Team De Groot") &
                        (raw_p_video["tutor"] != "Annelies de Groot")
                    ].copy()
    ...
    if page == "📹 Parent Update Videos":
        ...
        if _all_video_df.empty:
            st.warning("No parent update rows found for Team De Groot.")
            st.stop()
        ...
        st.download_button(
            ...
            data=out_v,
            file_name=f"Parent_Update_Videos_{sel_tutor_v.replace(' ','_') if sel_tutor_v != 'All Tutors' else 'Annelies_de_Groot'}.xlsx",
...
**Multiple tutors** is based on the last 30 days. When filtering to "Solo Tutor Only", only students where this tutor is their **only** tutor are shown.
        ...
        with tab_detail:
            ...
            if view_grades_df.empty:
                ...
                st.download_button(
                    ...
                    data=output_g,
                    file_name=f"Grades_Detail_{sel_tutor_g.replace(' ','_') if sel_tutor_g != 'All Tutors' else 'Annelies_de_Groot'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if page == "Test Prep & Exams":
        ...
        try:
            _report_path = "data/exam_reports/Annelies_de_Groot_exam_report.pdf"
        ...
        with tab_det_e:
            if view_exam_df.empty:
                ...
                st.download_button(
                    ...
                    data=out_e,
                    file_name=f"Exam_Detail_{sel_tutor_e.replace(' ','_') if sel_tutor_e != 'All Tutors' else 'Annelies_de_Groot'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
**Tutors with no updates sent that week will appear with blank scores.**
        ...
        if team_scores_df.empty:
            st.warning(f"No scored progress updates found for Team de Groot.")
            st.stop()
        ...
        st.download_button(
            ...
            data=out_qs,
            file_name=f"Update_Quality_Scores_{sel_tutor.replace(' ','_') if sel_tutor != 'All Tutors' else 'Annelies_de_Groot'}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    ...
    if page == "Archivable Students & Unscheduled Hours":
        ...
        if full_team_df.empty:
            st.warning("No records found for Team De Groot."); st.stop()
        ...
        with tab1:
            ...
            if archive_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Archivable_Students_Annelies_de_Groot.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        with tab2:
            ...
            if unsched_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Unscheduled_Hours_Annelies_de_Groot.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        if not nps_start_val:
            ...
            try:
                with st.spinner("Loading NPS data..."):
                    nps_df = load_nps_scores(nps_start_val, nps_end_val, "Team De Groot")
        ...
        if not pu_as_of_val:
            ...
            try:
                with st.spinner("Loading progress update data..."):
                    pu_df = load_progress_updates(pu_as_of_val, pu_since_val, "Team De Groot")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                pu_df = pd.DataFrame()
    ...
    if page == "🚫 Restricted Status":
        ...
        try:
            ...
            restr_df = restr_df[restr_df["faculty_leader"] == "Annelies de Groot"].copy()
        except Exception as e:
    ...
    if page == "📅 Meetings":
        ...
        try:
            ...
            mtg_df = mtg_df[mtg_df["faculty_leader"] == "Annelies de Groot"].copy()
        except Exception as e:
        ...
        if not ppw_start:
            ...
            try:
                with st.spinner("Loading PPW data..."):
                    ppw_df = load_ppw_data(ppw_start, ppw_end, "Team De Groot")
                st.caption(f"Debug: {len(ppw_df)} rows, dates {ppw_start} to {ppw_end}, team: Annelies de Groot")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                ppw_df = pd.DataFrame()
            if ppw_df.empty:
        ...
        if selected_annual_tutor:
            ...
            if not tutor_review.empty:
                ...
                team_monthly_metric_df = monthly_metric_annual_review_df[
                    monthly_metric_annual_review_df["Faculty Leader"] == "Annelies de Groot"]
    ...
    # ─────────────────────────────────────────────────────────────────────────
        ...
        team_tutors_df = master_tutor_df[
            (master_tutor_df["Faculty Leader"] == "Annelies de Groot") &
            (~master_tutor_df["Full Name"].isin(["Annelies de Groot"]))
        ].copy()

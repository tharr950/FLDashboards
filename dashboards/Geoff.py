...
SNAPSHOT_HISTORY_FILE = "geoff_archive_snapshots_history.csv"
GRADES_SNAPSHOT_FILE = "geoff_grades_snapshots.csv"
EXAMS_SNAPSHOT_FILE  = "geoff_exams_snapshots.csv"
VIDEO_SNAPSHOT_FILE  = "geoff_video_snapshots.csv"
PROGRESS_SNAPSHOT_FILE = "geoff_progress_score_snapshots.csv"
...
WATCHLIST_FILE            = "geoff_watchlist.csv"
WATCHLIST_BASELINE_FILE   = "geoff_watchlist_baselines.csv"
WATCHLIST_NOTES_FILE      = "geoff_watchlist_notes.csv"
WATCHLIST_THRESHOLDS_FILE = "geoff_watchlist_thresholds.csv"
...
LOGIN_SNAPSHOT_FILE = "geoff_login_snapshot.csv"
LAST_SEEN_FILE      = "geoff_last_seen_data.csv"
...
def render_app(config):
    ...
    annelies_tutors = master_tutor_df[(master_tutor_df["Faculty Leader"] == "Geoff St. Marie") & (~master_tutor_df["Full Name"].isin(["Ela Cross', 'Annelies de Groot', 'Ian Plamondon', 'Geoff St. Marie', 'Kristin Haase-Alvey"]))]["Full Name"].dropna().sort_values().tolist()
    ...
    if page == "🏠 Home":
        st.markdown('<div class="main-title">Good morning, Geoff 👋</div>', unsafe_allow_html=True)
        ...
        with st.spinner("Loading your briefing..."):
            ...
            try:
                ...
                home_arch_df = raw_arch_df[raw_arch_df["team_name"] == "Team St. Marie"].copy()
            except Exception as e:
                home_arch_df = pd.DataFrame(); load_errors.append(f"Archivable data: {e}")

            try:
                raw_home_grades, _ = load_grades_data()
                home_grades_df = raw_home_grades[raw_home_grades["team_name"] == "Team St. Marie"].copy()
            ...
            try:
                ...
                home_exam_df = raw_home_exam[raw_home_exam["team_name"] == "Team St. Marie"].copy()
            ...
            try:
                ...
                team_avail_df = avail_df[avail_df["team"] == "Team St. Marie"].copy()
            ...
            try:
                ...
                home_video_df = raw_video[
                    (raw_video["faculty leader"] == "Team St. Marie") &
                    (raw_video["tutor"] != "Geoff St. Marie")
                ].copy()
        ...
        if not home_arch_df.empty:
            ...
            if team_avail_df.empty:
                ...
                <div style='background:#f0fff4; border:1.5px solid #b2f5c8; border-radius:10px;
                    ...
                    ✅ <b>Availability Looks Good</b> — No tutors on Team St. Marie have 7+ days
        ...
        try:
            _low_avail_df = load_low_delivery_low_availability("Geoff St. Marie")
        ...
        try:
            ...
            if not _featured_df.empty:
                _my_featured = _featured_df[_featured_df["faculty_leader"] == "Geoff St. Marie"].copy()
        ...
        if not kpi_home_df.empty:
            ...
            latest_team = kpi_home_df[
                ...
                (kpi_home_df["Faculty Leader"] == "Geoff St. Marie")]
            prev_team   = kpi_home_df[
                (kpi_home_df["Date Range Parsed"] == prev_kpi) &
                (kpi_home_df["Faculty Leader"] == "Geoff St. Marie")] if pd.notna(prev_kpi) else pd.DataFrame()
            for m in kpi_metrics:
        ...
        _intro = (f"Here's your briefing for **{_today_str}**. "
                  f"Team St. Marie has **{_total_active} active students**, "
...
# ─────────────────────────────────────────────
    ...
    if page == "👀 Watched Tutors":
        ...
        all_tutors_wl = sorted(
            master_tutor_df[master_tutor_df["Faculty Leader"] == "Geoff St. Marie"]["Full Name"]
        ...
        with st.spinner("Loading watch list data..."):
            try:
                ...
                raw_wl_arch["should_archive"] = raw_wl_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                wl_arch_df = raw_wl_arch[raw_wl_arch["team_name"] == "Team St. Marie"].copy()
            except Exception as e:
                wl_arch_df = pd.DataFrame(); load_errors_wl.append(f"Archivable: {e}")

            try:
                raw_wl_grades, _ = load_grades_data()
                wl_grades_df = raw_wl_grades[raw_wl_grades["team_name"] == "Team St. Marie"].copy()
            ...
            try:
                ...
                wl_exam_df = raw_wl_exam[raw_wl_exam["team_name"] == "Team St. Marie"].copy()
                for dc in ["first_session_day","most_recent_session","exam_date"]:
            ...
            try:
                ...
                wl_video_df = raw_wl_video[
                    (raw_wl_video["faculty leader"] == "Team St. Marie") &
                    (raw_wl_video["tutor"] != "Geoff St. Marie")
                ].copy()
    ...
    if page == "👤 Tutor Profile":
        ...
        try:
            _ld_flag_df = load_low_delivery_not_accepting("Geoff St. Marie")
        ...
        with st.spinner(f"Loading data for {profile_tutor}…"):
            try:
                ...
                raw_p_arch["should_archive"] = raw_p_arch["should_archive"].apply(
                    lambda x: bool(x) if pd.notna(x) else False)
                p_arch = raw_p_arch[
                    (raw_p_arch["team_name"] == "Team St. Marie") &
            ...
            try:
                ...
                p_grades = raw_p_grades[
                    (raw_p_grades["team_name"] == "Team St. Marie") &
            ...
            try:
                ...
                p_exam = raw_p_exam[
                    (raw_p_exam["team_name"] == "Team St. Marie") &
            ...
            try:
                ...
                p_video_df = raw_p_video[
                    (raw_p_video["faculty leader"] == "Team St. Marie") &
                ...
                if not p_video_df.empty:
                    all_video = raw_p_video[
                        (raw_p_video["faculty leader"] == "Team St. Marie") &
                        (raw_p_video["tutor"] != "Geoff St. Marie")
                    ].copy()
        ...
        if p_monthly_t.empty:
            ...
            team_names_p   = p_master[p_master["Faculty Leader"] == "Geoff St. Marie"]["Full Name"].dropna()
    ...
    if page == "📹 Parent Update Videos":
        ...
        st.download_button(
            ...
            data=out_v,
            file_name=f"Parent_Update_Videos_{sel_tutor_v.replace(' ','_') if sel_tutor_v != 'All Tutors' else 'Geoff_St_Marie'}.xlsx",
...
**Multiple tutors** is based on the last 30 days. When filtering to "Solo Tutor Only", only students where this tutor is their **only** tutor are shown.
        ...
        with tab_detail:
            ...
            if view_grades_df.empty:
                ...
                st.download_button(
                    ...
                    data=output_g,
                    file_name=f"Grades_Detail_{sel_tutor_g.replace(' ','_') if sel_tutor_g != 'All Tutors' else 'Geoff_St_Marie'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if page == "Test Prep & Exams":
        ...
        try:
            _report_path = "data/exam_reports/Geoff_St_Marie_exam_report.pdf"
...
- If test prep is covered in an **Academics session**: it is counted in delivered sessions, but Academics are not included for sessions scheduled in the future
        ...
        team_exam_df = raw_exam_df[raw_exam_df["team_name"] == "Team St. Marie"].copy()
        if team_exam_df.empty:
            st.warning("No exam records found for Team St. Marie.")
            st.stop()
        ...
        with tab_det_e:
            if view_exam_df.empty:
                ...
                st.download_button(
                    ...
                    data=out_e,
                    file_name=f"Exam_Detail_{sel_tutor_e.replace(' ','_') if sel_tutor_e != 'All Tutors' else 'Geoff_St_Marie'}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
**Tutors with no updates sent that week will appear with blank scores.**
        ...
        if team_scores_df.empty:
            st.warning(f"No scored progress updates found for Team St. Marie.")
            st.stop()
        ...
        st.download_button(
            ...
            data=out_qs,
            file_name=f"Update_Quality_Scores_{sel_tutor.replace(' ','_') if sel_tutor != 'All Tutors' else 'Geoff_St._Marie'}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    ...
    if page == "Archivable Students & Unscheduled Hours":
        ...
        if full_team_df.empty:
            st.warning("No records found for Team St. Marie."); st.stop()
        ...
        with tab1:
            ...
            if archive_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Archivable_Students_Geoff_St_Marie.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        with tab2:
            ...
            if unsched_df.empty:
                ...
                st.download_button(
                    ...
                    file_name=f"Unscheduled_Hours_Geoff_St_Marie.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        if not nps_start_val:
            ...
            try:
                with st.spinner("Loading NPS data..."):
                    nps_df = load_nps_scores(nps_start_val, nps_end_val, "Team St. Marie")
        ...
        if not pu_as_of_val:
            ...
            try:
                with st.spinner("Loading progress update data..."):
                    pu_df = load_progress_updates(pu_as_of_val, pu_since_val, "Team St. Marie")
            except Exception as _e:
                st.error(f"Query error: {_e}")
                pu_df = pd.DataFrame()
    ...
    if page == "🚫 Restricted Status":
        ...
        try:
            ...
            restr_df = restr_df[restr_df["faculty_leader"] == "Geoff St. Marie"].copy()
        except Exception as e:
    ...
    if page == "📅 Meetings":
        ...
        try:
            ...
            mtg_df = mtg_df[mtg_df["faculty_leader"] == "Geoff St. Marie"].copy()
        except Exception as e:
        ...
        if selected_annual_tutor:
            ...
            if not tutor_review.empty:
                ...
                team_monthly_metric_df = monthly_metric_annual_review_df[
                    monthly_metric_annual_review_df["Faculty Leader"] == "Geoff St. Marie"]
    ...
    if page == "KPI Table":
        ...
        if progress_df.empty:
            ...
            leader_to_team = {
                ...
                "Geoff St. Marie":     "Team St. Marie",
                "Annelies de Groot":   "Team St. Marie"
            }
